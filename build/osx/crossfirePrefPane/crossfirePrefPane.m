...
#import "crossfirePrefPane.h"

@implementation crossfirePrefPane
...
- (BOOL)isRunning
{
    ...
    if((ps=popen(pscmd, "r")) == NULL)
    {
        ...
        NSBeginAlertSheet(
            ...
            nil,
            self,
            @"An error occured while detecting a running crossfire process.",
            nil);
...
- (IBAction)toggleAutoStart:(id)sender
{
	...
	if(ourStatus != errAuthorizationSuccess)
	{
		...
		NSBeginAlertSheet(
						  ...
						  nil,
						  self,
						  @"Could not toggle crossfire startup.",
						  nil);
...
- (void)updateStatus
{
	if ([self isRunning] == NO)
	{
		...
		[startButton setTitle:@"Start crossfire"];
	...
	else 
	{
		...
		[startButton setTitle:@"Stop crossfire"];
...
- (void)startServer
{
	...
	if(ourStatus != errAuthorizationSuccess)
	{
		...
		NSBeginAlertSheet(
			...
			nil,
			self,
			@"Could not start the crossfire server.",
			nil);
		[statusTimer invalidate];
		[self checkStatus];
	}
...
- (void) stopServer
{
	...
	if(ourStatus != errAuthorizationSuccess)
	{
		...
		NSBeginAlertSheet(
			...
			nil,
			self,
			@"Could not stop the crossfire server.",
			nil);
		[statusTimer invalidate];
		[self checkStatus];
	}
