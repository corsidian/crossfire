...
static bool crossfireIsRunning()
{
    FILE *ps;
    char buff[1024];
    
    if((ps=popen("/bin/ps auxww | fgrep -v 'fgrep' | fgrep crossfire/lib/startup.jar", "r")) == NULL)
    {
        return false;
    }
    else
...
static void togglecrossfire()
{
	char *args[4];
	args[0] = "launchctl";
	if(crossfireIsRunning())
		args[1] = "unload";
	else
		args[1] = "load";
...
int main(int inArgsCount, char * const inArgs[])
{
	...
	const char* correctPath = "/Library/PreferencePanes/crossfire.prefPane/Contents/MacOS/HelperTool";
