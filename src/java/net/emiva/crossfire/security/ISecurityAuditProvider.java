...
/**
 * A ISecurityAuditProvider handles storage and retrieval of security audit logs.  If set to
 * write-only, the logs are not viewable from crossfire's admin console.  An optional URL
 * can be provided for the location of where the logs can be viewed.
 * 
 * @author Daniel Henninger
 */
public interface ISecurityAuditProvider {
    ...
    /**
     * Returns true if the provider logs can be read by crossfire for display from crossfire's
     ...
     * with audits it's own user events and does not need crossfire to duplicate the event.
     *
     ...
     * security events on through.  This is typically used if the service the provider is working
     * with audits it's own group events and does not need crossfire to duplicate the event.
     *
