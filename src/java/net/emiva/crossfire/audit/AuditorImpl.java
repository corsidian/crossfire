...
public class AuditorImpl implements IAuditor {
    ...
    private net.emiva.util.XMLWriter xmlWriter;
    /**
     ...
     */
    private Timer timer = new Timer("IAuditor");
    private SaveQueuedPacketsTask saveQueuedPacketsTask;
    private FastDateFormat dateFormat;

    public AuditorImpl(IAuditManager manager) {
    ...
    public void setLogTimeout(int logTimeout) {
        ...
        saveQueuedPacketsTask = new SaveQueuedPacketsTask(this);
    ...
    public void audit(Packet packet, ISession session) {
    ...
    private void writePacket(Packet packet, ISession session) {
        if (!closed) {
	...
	*/
	private void createAuditFile(Date auditDate) throws IOException {
   	...
   	filesIndex = Math.max(files.length, filesIndex);
		...
		xmlWriter = new net.emiva.util.XMLWriter(writer);
	}

    protected void saveQueuedPackets() {
