...
import org.b5chat.crossfire.xmpp.session.SessionEventDispatcher;
 ...
 */
public class IQAuthHandler extends IQHandler implements IIQAuthInfo {
	...
	public IQ handleIQ(IQ packet) throws UnauthorizedException, PacketException {
        ...
        if (session == null) {
            Log.error("Error during authentication. ISession not found in " +
            ...
            try {
                ...
                // Otherwise set query
                    ...
                    if (query.elements().isEmpty()) {
                        ...
                        resourceBound = session.getStatus() == ISession.STATUS_AUTHENTICATED;
                    }
                    else {
                        String username = query.elementText("username");
    ...
    private IQ login(String username, Element iq, IQ packet, String password, LocalClientSession session, String digest)
        ...
        IClientSession oldSession = routingTable.getClientRoute(new JID(username, serverName, resource, true));
        if (oldSession != null) {
            try {
    ...
    private IQ passwordReset(String password, IQ packet, String username, ISession session)
    ...
    private IQ anonymousLogin(LocalClientSession session, IQ packet) {
        ...
        if (anonymousAllowed) {
            ...
            if (forbidAccess) {
                // IConnection forbidden from that IP address
                response.setChildElement(packet.getChildElement().createCopy());
                response.setError(PacketError.Condition.forbidden);
            }
            else {
