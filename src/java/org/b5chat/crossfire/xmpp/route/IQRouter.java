...
import org.b5chat.crossfire.core.util.TaskEngine;
...
public class IQRouter extends BasicModule {
    ...
    public void route(IQ packet) {
        ...
        IClientSession session = sessionManager.getSession(sender);
        try {
            // Invoke the interceptors before we process the read packet
            InterceptorManager.getInstance().invokeInterceptors(packet, session, true, false);
            JID to = packet.getTo();
            if (session != null && to != null && session.getStatus() == ISession.STATUS_CONNECTED &&
            ...
            else if (session == null || session.getStatus() == ISession.STATUS_AUTHENTICATED || (
                    isLocalServer(to) && (
