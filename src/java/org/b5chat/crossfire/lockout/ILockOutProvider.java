...
/**
 * A ILockOutProvider handles storage of information about disabled accounts, and requests for
 * whether an account is currently disabled.  If set read-only, only requests for disabled
 * status are expected to work.
 *
 * @author Daniel Henninger
 */
public interface ILockOutProvider {
    ...
    /**
     * Returns true if this ILockOutProvider is read-only. When read-only,
     * disabled status of accounts can not be changed via crossfire.
     *
    ...
    /**
     * Returns true if the ILockOutProvider allows for a delayed start to the lockout.
    ...
    /**
     * Returns true if the ILockOutProvider allows for a timeout after which the lock out will expire.
     ...
     * target without crossfire knowing about it.
     *
